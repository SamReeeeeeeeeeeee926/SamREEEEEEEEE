...
public void setup() {
  ...
  //step 0
  introWord.add(Ani.to(this, 1.5f, "tY:200", Ani.EXPO_OUT));
...
public void draw() {
  if (gameS ==1) {
    ...
    if (introWord.isEnded()) {
      ...
      dia1 += sin(x/4)* 0.5f; //heartbeat1
      dia2 += sin(x/4)* 0.5f; //heartbeat2
      dia3 += sin(x/4)* 0.5f; //heartbeat3
    }
...
public void options() {
  ...
  dia11 += sin(x/4)* 0.5f; //heartbeat11
  dia22 += sin(x/4)* 0.5f; //heartbeat22
  x--;
  ...
  public void settings() {  size(712, 1000);  smooth(); }
